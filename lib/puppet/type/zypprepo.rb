# Description of zypp repositories
# zypprepo based on yumrepo type
# zypp (zypper) repos are used in SuSE linux and derivatives
...
module Puppet
  ...
  Puppet::Type.newtype(:zypprepo) do
        @doc = "The client-side description of a yast repository. Repository
                configurations are found by parsing /etc/zpp/zypp.conf and
                the files indicated by reposdir in that file (see zypp.conf(5)
                for details)

                Most parameters are identical to the ones documented
                in zypp.conf(5)

                Continuation lines that zypp supports for example for the
        ...
        class << self
            ...
            # to change zyppconf or inifile in any other context
            attr_accessor :zyppconf
            attr_writer :inifile
        end
        ...
        # Return the Puppet::Util::IniConfig::File for the whole zypp config
        def self.inifile
            if @inifile.nil?
                @inifile = read()
                main = @inifile['main']
                if main.nil?
                    raise Puppet::Error, "File #{zyppconf} does not contain a main section"
                end
        ...
        newproperty(:enablegroups, :parent => Puppet::IniProperty) do
            desc "Determines whether zypp will allow the use of
        ...
        newproperty(:keepalive, :parent => Puppet::IniProperty) do
            desc "Either '1' or '0'. This tells zypp whether or not HTTP/1.1
        ...
        newproperty(:type, :parent => Puppet::IniProperty) do
           desc "Type for this repository.\n#{ABSENT_DOC}"
           newvalue(:absent) { self.should = "None" }
           newvalue(/.*/) { }
        end
        
        newproperty(:autorefresh, :parent => Puppet::IniProperty) do
           desc "Enable or disable autorefresh for this repository.#{ABSENT_DOC}"
           newvalue(:absent) { self.should = "0" }
           newvalue(%r{(0|1)}) { }
        end

        newproperty(:path, :parent => Puppet::IniProperty) do
           desc "Path for this repository.\n#{ABSENT_DOC}"
           newvalue(:absent) { self.should = "/" }
           newvalue(/.*/) { }
        end

        newproperty(:keeppackages, :parent => Puppet::IniProperty) do
          desc "Enable RPM caching this repository.#{ABSENT_DOC}"
          newvalue(:absent) { self.should = "0" }
          newvalue(%r{(0|1)}) { }
        end
    end
